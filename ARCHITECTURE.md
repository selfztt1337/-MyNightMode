...
9. `ScheduleEngine` — чистая функция времени: выбирает последнее событие и ближайший переход без геолокации или сети.
10. Пользовательские пресеты и профили дисплеев кодируются через `Codable`; старые конфигурации получают безопасные значения новых полей.
11. Диагностика сравнивает frame панели с полным `NSScreen.frame`; экспорт использует нейтральную нумерацию дисплеев и исключает их пользовательские имена.
